...
export function validateForm() {
    ...
    if (data.date && new Date(`${data.date}T00:00:00`) > new Date().setHours(0, 0, 0, 0)) {
        showError('date', 'Date cannot be in the future');
        valid = false;
    }

    if (!Number.isInteger(data.timeSpent) || data.timeSpent < 0) { showError('timeSpent', 'Enter a valid time'); valid = false; }
...
export function clearErrors() {
    ...
    document.querySelectorAll('.error-input').forEach((element) => element.classList.remove('error-input'));
}
...
export function showPageMismatchError(existingPages, newPages) {
    showError('totalPages', `This book is already recorded with ${existingPages} pages. You cannot change it to ${newPages} pages.`);
}
